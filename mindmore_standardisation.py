...
class mindmore_standardiser:
    ...
    def Stroop_index_standardizer(self):
        ...
        def Stroop_standardizer(row):
            ...
            if row['GENERAL_inputDevice'] == 1:
                ...
                return (row['STROOP_INCONGRUENT_INDEX'] - Stroop_PREDICTED) / SD
    ...
    def Stroop_inhibition_standardiser(self):
        ...
        def Stroop_standardizer_SD_picker(row):
            ...
            if (STROOP_INTERFERENCE_predicted < 296.210):
                ...
                return -(row['STROOP_INTERFERENCE'] - \
                    STROOP_INTERFERENCE_predicted) / 199.392
                    
            elif (STROOP_INTERFERENCE_predicted > 296.210 and 
                  STROOP_INTERFERENCE_predicted < 406.837):
    
                return -(row['STROOP_INTERFERENCE'] - \
                    STROOP_INTERFERENCE_predicted) / 223.075
    
            elif (STROOP_INTERFERENCE_predicted > 406.836 and 
                  STROOP_INTERFERENCE_predicted < 517.463):
    
                return -(row['STROOP_INTERFERENCE'] - \
                    STROOP_INTERFERENCE_predicted) / 272.525
    
            elif STROOP_INTERFERENCE_predicted >= 517.463:
    
                return -(row['STROOP_INTERFERENCE'] - \
                    STROOP_INTERFERENCE_predicted) / 388.589
                    
        self.df['STROOP_INTERFERENCE_z'] = self.df.apply(lambda row: Stroop_standardizer_SD_picker(row), axis=1)
